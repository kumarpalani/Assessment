...
function changeHeading() {
  /* Case where the language index has gone out of bounds of the array; reshuffle array and
       reset text */
  console.log("test", jsonData.contentArr.length);
  if (languageIndex === jsonData.contentArr.length) {
    clearInterval(fadeInterval);
    endFrame();
  } else {
    // Iterate through the random language selected, changing each DOM element sequentially
    fadeAnimation();
    const language = jsonData.contentArr[languageIndex];
    landingTitle.textContent = language;
    landingTitle.style.color = jsonData.textColor[languageIndex];
